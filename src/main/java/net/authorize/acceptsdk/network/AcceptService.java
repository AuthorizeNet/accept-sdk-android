...
public class AcceptService extends IntentService {
     ...
     * @see IntentService
     */
    public static void startActionEncrypt(Context context, final EncryptTransactionObject transactionObject, TransactionResultReceiver resultReceiver) {
    ...
    protected void onHandleIntent(Intent intent) {
        if (intent != null) {
            ...
            switch (action){
                case ACTION_ENCRYPT:
                    final EncryptTransactionObject transactionObject = (EncryptTransactionObject)
                            intent.getSerializableExtra(EXTRA_PARAM_TRANSACTION_OBJECT);
    ...
    private Object handleActionEncrypt(EncryptTransactionObject transactionObject) {
        ...
        try {
            ...
            if (responseCode == HttpsURLConnection.HTTP_OK) {
                EncryptTransactionResponse result = AcceptSDKParser.createEncryptionResponse(urlConnection.getInputStream());
                //FIXME : commented for testing purpose
              //  if (result.getReasonCode().equals(REASON_CODE_OK) ) {
                    resultObject = result;
